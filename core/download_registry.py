...
def _resolve_metadata(link, metadata=None, metadata_fetcher=None):
    if metadata:
        return metadata
...
def register_downloaded_link(
    ...
    resolved_metadata = _resolve_metadata(link, metadata, metadata_fetcher)
    if not resolved_metadata or not destination_root:
        return False

    destination_root = Path(destination_root)
    matched_files = find_matching_media_files(resolved_metadata, destination_root)
...
def should_skip_download(
    ...
    resolved_metadata = _resolve_metadata(link, metadata, metadata_fetcher)
    if is_already_downloaded(resolved_metadata, destination_root):
