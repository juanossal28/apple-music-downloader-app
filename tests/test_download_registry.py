...
def test_register_downloaded_link_fetches_metadata_when_missing(tmp_path: Path):
    destination_root = tmp_path / 'downloads'
    registry_path = tmp_path / 'registry.json'
    song_path = destination_root / 'Artist' / 'Album' / '04 Remember.m4a'
    song_path.parent.mkdir(parents=True)
    song_path.write_text('data', encoding='utf-8')

    link = 'https://music.apple.com/mx/album/remember/1537902814?i=1537902977'
    ...
    assert register_downloaded_link(
        ...
        destination_root,
        registry_path,
        metadata_fetcher=lambda current_link: {
            'artist': 'Artist',
            'album': 'Album',
            'track': 'Remember',
            'track_number': 4,
...
def test_should_skip_download_fetches_metadata_when_missing(tmp_path: Path):
    destination_root = tmp_path / 'downloads'
    registry_path = tmp_path / 'registry.json'
    song_path = destination_root / 'Artist' / 'Album' / '04 Remember.m4a'
    song_path.parent.mkdir(parents=True)
